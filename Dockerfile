...
WORKDIR /build/ecommerce-go-order-service
...
COPY --from=builder /build/ecommerce-go-order-service/main .

# Copy config files if they exist
COPY --from=builder /build/ecommerce-go-order-service/config* ./config/ 2>/dev/null || true
